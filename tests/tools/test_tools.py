...
from vkbottle.exception_factory.base_exceptions import APIAuthError
...
@pytest.mark.asyncio
async def test_user_auth_with_2fa_app(mock_aioresponse: aioresponses):
    mock_aioresponse.post(
        "https://oauth.vk.com/token",
        payload={
    ...
    assert exc_info.value.validation_type == "2fa_app"
