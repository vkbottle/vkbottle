...
from vkbottle.dispatch.rules.user import (
    AttachmentTypeRule,
    ChatActionRule,
    CommandRule,
    CoroutineRule,
...
from vkbottle.dispatch.views import ABCView, UserHandlerBasement, UserMessageView, UserRawEventView
...
class UserLabeler(ABCUserLabeler):
    ...
    def __init__(self, **kwargs):
        ...
        self.raw_event_view = UserRawEventView()
    ...
    def raw_event(
        ...
        **custom_rules,
        ...
        def decorator(func):
            for e in event:
                self.raw_event_view.handlers[e] = UserHandlerBasement(
                    dataclass,
                    FromFuncHandler(
                        func,
                        *map(convert_shorten_filter, rules),
