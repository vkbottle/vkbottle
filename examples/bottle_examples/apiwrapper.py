...
@bot.on.message("!кто я", lower=True)
async def wrapper(ans: Message):
    user = (
        await bot.api.users.get(
            user_ids=ans.from_id, fields=",".join(["status", "photo_id"])
        )
    )[0]
    await ans(
        f'Ты {user["first_name"]} {user["last_name"]}. Статус по жизни: <<{user["status"]}>>. '
